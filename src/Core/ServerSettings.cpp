...
#define LIST_OF_SERVER_SETTINGS_WITH_PATH(DECLARE, ALIAS) \
    ...
    DECLARE(UInt64, query_plan_cache_max_size_in_bytes, 536870912, R"(The maximum size of the query plan cache in bytes. 0 means the query plan cache is disabled.)", 0, "query_plan_cache.max_size_in_bytes") \
    DECLARE(UInt64, query_plan_cache_max_entries, 1024, R"(The maximum number of query plans stored in the cache.)", 0, "query_plan_cache.max_entries") \
