...
struct ChildrenSet2
{
    ...
    /// combineActions(pre-existing action, `action`, strict).
...
struct MemtableChildrenSet
{
    ...
    /// Calls out.insertCombine for each element of this set.
    void mergeInto(ChildrenSet2 & out, DB::Arena & arena, bool strict) const;
...
struct Memtable
{
    ...
    ///          least one node is appended to them. Because BlockWeakPtr in CommittedNodeInfo
    ...
    NodeRef appendNode(FullNode & node, int64_t zxid, bool strict);

    /// insertCombine children of this node into `out`, copying the names into `out_arena`.
    void listChildren(const NodePathWithHash & path, bool strict, ChildrenSet2 & out, DB::Arena & out_arena) const;
};
