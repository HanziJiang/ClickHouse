...
NodePathWithHash NodePath::calculateHash() const
{
    return NodePathWithHash{.path = *this, .hash = sipHash128(str, len)};
}
...
uint64_t FullNode::getOrCalculateDigest()
{
    ...
    /// The casts spell out the field widths that KeeperStorage hashes.
    SipHash hash;

    hash.update(std::string_view(path.str, path.len));
    if (!data.empty())
        hash.update(data);

    hash.update(static_cast<int64_t>(stats.czxid));
    hash.update(static_cast<int64_t>(stats.mzxid));
    hash.update(static_cast<int64_t>(stats.ctime));
    hash.update(static_cast<int64_t>(stats.mtime));
    hash.update(static_cast<int32_t>(basic.version));
    hash.update(static_cast<int32_t>(stats.cversion));
    hash.update(static_cast<int32_t>(stats.aversion));
    hash.update(static_cast<int64_t>(stats.getEphemeralOwner(basic.isEphemeral())));
    hash.update(static_cast<int32_t>(basic.getNumChildren()));
    hash.update(static_cast<int64_t>(stats.pzxid));
...
BlockPtr BlockData::create(size_t capacity_)
{
    ...
    /// shared_ptr with separately allocated control block (no make_shared) is what we want.
    /// Don't want weak_ptr to pin the whole BlockData+buffer, or even just BlockData.
    return BlockPtr(
        block,
        [](BlockData * b)
        {
...
void BlockData::writeHeader(BlockPtr & block, const FullNode & node)
{
    ...
    block->base_zxid = node.stats.mzxid;
    block->base_time = node.stats.mtime;
    ...
    memcpy(p, node.path.str, base_path_len);
...
NodeRef BlockData::appendNodeNoResize(BlockPtr block, FullNode & node)
{
    ...
    *p = char(uint8_t(node.basic.action));
    ...
    while (size_t(path_prefix_size) + 8 <= size_t(max_prefix) &&
           unalignedLoad<uint64_t>(base_path_str + path_prefix_size) == unalignedLoad<uint64_t>(node.path.str + path_prefix_size))
        path_prefix_size += 8;
    while (path_prefix_size < max_prefix && base_path_str[path_prefix_size] == node.path.str[path_prefix_size])
        ++path_prefix_size;
    const uint32_t path_suffix_size = node.path.len - path_prefix_size;
    const char * const path_suffix_str = node.path.str + path_prefix_size;
    ...
    if (node.basic.action != NodeAction::Remove)
    {
        DB::GroupVarint4x32::encode(
            p, node.stats.cversion, node.stats.aversion, 0, 0);
        DB::GroupVarint8x64::encode(
            p,
            encodeZigZagDelta64(block->base_zxid, node.stats.czxid),
            encodeZigZagDelta64(block->base_zxid, node.stats.mzxid),
            encodeZigZagDelta64(block->base_zxid, node.stats.pzxid),
            encodeZigZagDelta64(block->base_time, node.stats.ctime),
            encodeZigZagDelta64(block->base_time, node.stats.mtime),
            node.stats.ephemeral_owner_or_seq_num,
            0,
            0);
...
bool BlockData::shrinkIfSparse(BlockPtr & block)
{
    /// Shrink only if more than 20% of the capacity is unused.
    if (static_cast<size_t>(block->size) * 5 >= static_cast<size_t>(block->capacity) * 4)
        return false;

    BlockPtr new_block = create(block->size);
    const uint32_t cap = new_block->capacity;
