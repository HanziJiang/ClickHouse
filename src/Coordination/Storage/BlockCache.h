...
/// Cache of blocks (BlockData) read from files (FileState).
/// Accessed infrequently, usually to populate the nodes hash map in StorageState. Most individual
/// node lookups find a cached block through weak_ptr in StorageState or FileState, without touching
...
{
    size_t operator()(const BlockData & block) const;
};
...
private:
    DB::CacheBase<uint64_t, BlockData, DefaultHash<uint64_t>, BlockCacheWeightFunction> cache;
};
