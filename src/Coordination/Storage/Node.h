...
struct NodeStats2
{
    ...
    uint64_t getEphemeralOwner(bool is_ephemeral) const { return is_ephemeral ? ephemeral_owner_or_seq_num : 0; }
    uint64_t getSeqNum(bool is_ephemeral) const { return is_ephemeral ? 0 : ephemeral_owner_or_seq_num; }
};
...
/// The BlockData+buffer combination is always wrapped in shared_ptr<BlockData>.
///
...
struct BlockData
{
    ...
    /// If block has lots of unused capacity, shrink it (allocate a smaller block and memcpy into it).
    static bool shrinkIfSparse(BlockPtr & block);
    ...
    std::string_view * out_data) const
{
    ...
    if (action_byte > static_cast<uint8_t>(NodeAction::Remove))
    ...
    if (out_stats && out_basic.action != NodeAction::Remove)
    {
        ...
        DB::GroupVarint4x32::decode(varints, out_stats->cversion, out_stats->aversion, unused, unused);
        ...
        DB::GroupVarint8x64::decode(
            varints, czxid_delta, mzxid_delta, pzxid_delta, ctime_delta, mtime_delta, out_stats->ephemeral_owner_or_seq_num, unused64, unused64);

        out_stats->czxid = decodeZigZagDelta64(block->base_zxid, czxid_delta);
        out_stats->mzxid = decodeZigZagDelta64(block->base_zxid, mzxid_delta);
        out_stats->pzxid = decodeZigZagDelta64(block->base_zxid, pzxid_delta);
        out_stats->ctime = decodeZigZagDelta64(block->base_time, ctime_delta);
        out_stats->mtime = decodeZigZagDelta64(block->base_time, mtime_delta);
    }
    ...
    if (out_path)
    {
        ...
        out_path->str = out_path_buf->data();
    }
